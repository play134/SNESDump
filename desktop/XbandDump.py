...
def main():
    ...
    if sys.platform.startswith("win"):
        ports = ["COM" + str(i + 1) for i in range(15)]
    ...
    def print_options():
        print " u - Write q File To Enable Xband SRAM\n i - Cart Info\n d - Dump ROM\n s - Dump SRAM\n w - Write SRAM\n h - Show this screen\n q - Quit"
    print_options()
    ...
    while not quit:
        ...
        if action == "i":
            ...
            set_ctrl_lines(port, True, False, False, True)
            ...
            end_addr = start_addr + file_size  - 1
            byte_val = 0x01
            packed_byte = struct.pack('B', byte_val)
            port.write(commands['WRITESECTION'])
            # write bank number
            port.write(chr(bank))
            # write start and end addresses
            ...
            #while input_file.tell() < file_size:
                #this_byte = input_file.read(1)
            port.write(packed_byte)
            bytes_written += 1
            time.sleep(0.001) #add a small delay
                #sys.stdout.write("\r Writing SRAM {0}/{1} bytes".format(bytes_written, sram_size))
            sys.stdout.write("\r Enabling X-BAND SRAM {0}/{1} bytes".format(bytes_written, sram_size))
            sys.stdout.flush()
